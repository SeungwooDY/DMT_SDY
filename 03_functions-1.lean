...
Answer here: if the function takes an argument *x*
             and another "free" or external argument *y*,
             then alpha conversion would incorrectly
             change the value/meaning of the variable
...
def myApply : (Nat → Nat → Nat) → Nat → Nat → Nat
| f, x, y => f x y

#eval myApply (fun x y => x + y) 2 3
...
Answer here: the type of this function is a function
             from any given α → α (Type → Type)
...
Answer: the type of this function is a function
        from any given (α : Type) → (α → α)
