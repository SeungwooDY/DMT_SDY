...
Answer here: it is wrong when y already exists as a
free variable in the body of the function.
...
def add2 (n1 : Nat) (n2 : Nat) : Nat := n1 + n2

def applyy (f : (Nat → Nat → Nat)) (x : Nat) (y : Nat) : Nat :=
  f x y

#eval applyy add2 2 3
